...
export const getAlertPaymentItems = (alert: AlertItem) => {
...
export const getPrimaryAlertPaymentItem = (alert: AlertItem): PaymentItem | null => getAlertPaymentItems(alert)[0] ?? null;

export const getAlertScheduledAt = (alert: AlertItem) => {
...
export const getAlertSentAt = (alert: AlertItem) => alert.sentAt || alert.createdAt || alert.updatedAt || null;
...
export const getAlertAssociatedLabel = (alert: AlertItem) => {
  const paymentItem = getPrimaryAlertPaymentItem(alert);
  if (!paymentItem) {
